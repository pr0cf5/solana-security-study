...
(async () => {
    ...
    if ( (await connection.getBalance(richBoy.publicKey)) < LAMPORTS_PER_SOL )
    {
    ...
    {
        const ix = Wallet.initialize(walletProgramId, authority);
    ...
    {
        const ix = Wallet.deposit(walletProgramId, authority, richBoy.publicKey, 42 * LAMPORTS_PER_SOL);
        let tx = new Transaction().add(ix);

...
function walletInitialize(walletProgramId, authority) {
    ...
    const keys = [
        walletMeta, authorityMeta, sysvarRentMeta, systemProgramMeta
    ];
...
function walletDeposit(walletProgramId, authority, source, amount) {
    ...
    const keys = [
        walletMeta, sourceMeta, systemProgramMeta
    ];
    let instruction = new TransactionInstruction({
        keys,
        programId: walletProgramId,
...
function walletWithdraw(walletProgramId, walletAddress, authorityPublicKey, dest, amount) {
    const walletMeta = AccountMeta(walletAddress, false);
    const authorityMeta = AccountMeta(authorityPublicKey, false);
    const destMeta = AccountMeta(dest, true);
    const keys = [
        walletMeta, authorityMeta, destMeta
    ];
    let instruction = new TransactionInstruction({
        keys,
        programId: walletProgramId,
